    ...
    <div class="text-[#0f2d2a] font-sans overflow-hidden">
        ...
        <section class="pt-32 pb-20 relative overflow-hidden bg-[#eaf0ee]/30 border-b border-[#0c5f56]/10">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 reveal">
                <span class="inline-block px-4 py-1.5 rounded-full bg-white border border-[#0c5f56]/10 text-[#0c5f56] text-[11px] font-bold uppercase tracking-wider mb-6">Workflow</span>
                    ...
                    <div class="reveal grid md:grid-cols-2 gap-16 items-center mb-28 {{ $i % 2 !== 0 ? 'md:grid-flow-dense' : '' }}">
                        <div class="{{ $i % 2 !== 0 ? 'md:col-start-2' : '' }}">
                            <div class="flex items-center gap-4 mb-6">
                                ...
                                <div class="h-px flex-1 bg-[#0c5f56]/10"></div>
                            </div>
                            <h3 class="font-serif text-3xl md:text-4xl text-[#0f2d2a] mb-4 leading-tight">
                                {{ $step['title'] }}
                            </h3>
                            <p class="text-[#0f2d2a]/80 font-semibold leading-relaxed mb-6">{{ $step['desc'] }}</p>
                            
                            <div class="grid grid-cols-2 gap-4">
                                @foreach($step['features'] as $feat)
                                    <div class="flex items-center gap-2.5 text-sm font-semibold text-[#0f2d2a]/95">
                        ...
                        <div class="relative {{ $i % 2 !== 0 ? 'md:col-start-1' : '' }}">
                            <div class="absolute -inset-4 bg-gradient-to-tr from-brand-300/10 to-teal-300/10 blur-2xl rounded-[3rem] opacity-60"></div>
                            <div class="bg-white/80 backdrop-blur-sm rounded-[2.5rem] p-10 flex flex-col items-center justify-center min-h-[300px] border border-[#0c5f56]/10 shadow-lg group hover:shadow-xl transition-shadow duration-500">
                                ...
                                <p class="text-xs font-bold text-[#0c5f56] uppercase tracking-widest">Step {{ $step['step'] }}</p>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        <section class="py-28 bg-[#0c5f56] text-center text-white relative overflow-hidden">
            ...
            <div class="max-w-3xl mx-auto px-4 reveal relative z-10">
                <h2 class="font-serif text-5xl md:text-6xl text-white mb-6">Ready to <span class="text-teal-200">Get Started?</span></h2>
                <p class="text-teal-100 mb-10 text-lg md:text-xl font-medium">See how simple it is to transform your lab operations.</p>
                <div class="flex flex-col sm:flex-row justify-center gap-4">
                    <a href="#contact" class="px-10 py-5 bg-white text-[#0c5f56] rounded-full font-bold text-lg shadow-xl shadow-teal-900/10 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                        Contact Us
                    </a>
                    <a href="{{ route('enquiry') }}" class="px-10 py-5 bg-[#094d45] border border-teal-500/30 text-white rounded-full font-bold text-lg transition-all duration-300 transform hover:-translate-y-1">
                        Request Demo
                    </a>
                </div>
            </div>
